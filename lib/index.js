...
function getNewerVersion(a, b, opts) {
    if (a === null) {
        return null;
    }
    ...
    for (let i = 0; i < aParts.length; i++) {
        if (i === 0 && opts?.breakOnMajorVersion) {
            if (aParts[0] !== bParts[0]) {
                ...
                console.log(`${opts?.packageName} major versions ${a} and ${b} are unresolvable.`);
                return null;
            }
        }
...
function newestVersionFromList(versions, opts) {
...
const rnModuleExpression = /ReactContextBaseJavaModule|RCTBridgeModule|ReactPackage|NativeModule/;
async function isRnPackage(pkgName) {
    const cwd = require$$0$1.join("node_modules", pkgName);
    ...
    for (const file of files) {
        const content = fs$8.readFileSync(file, "utf8");
...
async function findRnPackages(packages) {
    ...
    return rnPackages.filter(Boolean);
}
function findPackageDuplicates(packageMap) {
    ...
    console.log(`Found ${multipleVersions} duplicate package versions out of ${packageMap.size}`);
    return packages;
}
function parseYarnLockForPackages() {
    const file = fs$8.readFileSync("yarn.lock", "utf8");
...
function findNotHoistedRnPackages(rnPackages) {
    const notHoisted = rnPackages
        .map((packageName) => {
        // console.log(`Checking if ${packageName} is hoisted to root node_modules`);
        if (!fs$8.existsSync(`node_modules/${packageName}`))
            return packageName;
    })
        .filter(Boolean);
    return notHoisted;
}
function updateResolutions(duplicatePackages, packageMap) {
    let shouldRerunYarn = false;
    const packageJsonPath = require$$0$1.resolve("package.json");
    const packageJson = JSON.parse(fs$8.readFileSync(packageJsonPath, "utf8"));
    ...
    if (shouldRerunYarn) {
        fs$8.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
...
function getYarnListPackages(opts) {
    const { depth = 0 } = opts;
    let cmdOpts = "--json";
    if (depth) {
        cmdOpts += ` --depth=${depth}`;
    ...
    const packages = parsed.data.trees
        .filter((pkg) => pkg.depth <= depth)
        .map((pkg) => {
        const split = pkg.name.split("@");
        if (split[0] === "")
            return `@${split[1]}`; // Had leading @, add it back and return the name
        if (split[0] !== "")
            return split[0]; // No leading @, just reuturn name
    })
        .filter((v, i, array) => array.indexOf(v) === i); // unique
    return packages;
}
...
if (flag === "--write") {
    ...
    if (notHoisted.length > 0) {
        console.log("Following RN packages not found in root node_modules, should these be installed?");
    ...
    if (duplicateRnPackages.length > 1) {
        ...
        if (unresolveablePackagesFound) {
            console.warn("warning: There are packages that cannot be automatically resolved using resolutions.");
            console.log("Please use `yarn why {packageName}` to understand why they are required.");
        }
    }
    else {
        console.log("It appears everything is in order, no duplicated RN packages found.");
    }
}
